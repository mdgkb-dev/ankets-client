<template>
  <div class="an-v">
    ...
    <draggable :list="field.answerVariants" item-key="id" @end="updateOrder">
...
const addAnswerVariant = async () => {
  const item = props.field.addAnswerVariant();
  await AnswerVariantsStore.Create(item);
};

const removeAnswerVariant = async (id?: string) => {
  ClassHelper.RemoveFromClassById(id, props.field.answerVariants);
  await AnswerVariantsStore.Remove(id);
};

const updateAnswerVariant = async (item?: AnswerVariant) => {
  await AnswerVariantsStore.Update(item);
};
