<template>
  <div class="select-block">
    ...
    <div class="item-list">
      <div v-for="variant in question.questionVariants.filter((q) => !!researchResult.getQuestionVariantAnswer(q.id))"
        :key="variant.id" class="item-line">
        <div class="left">
          <div class="item-counter">
            <NumberProp v-if="question.valueType.isNumber()" :research-result="researchResult" :question="question"
              :variant-id="variant.id" @fill="$emit('fill')" />
          </div>
          <div class="item-name">{{ variant.name }}</div>
        </div>
        <div class="button">
          <Button text="Удалить" width="100%" height="40px" font-size="16px" border-radius="5px" color="#B0A4C0"
            background="#ffffff" :with-icon="false" :color-swap="true" @click="removeVariantAnswer(variant.id)">
          </Button>
        </div>
      </div>
    </div>
        ...
        <template #header>
          <div class="header-block">
            <div class="researche-title">
              <Button text="Назад" background="#ffffff" margin-right="10px" height="42px" font-size="16px"
                border-radius="5px" color="#343e5c" @click="toggleAddAllergen">
                <template #icon>
                  <svg class="icon-back">
        ...
        <template #body>
          <div class="body">
            <GridContainer grid-gap="5px" margin="10px 0">
              <Button v-for="variant in filteredVariants" :key="variant.id" :text="variant.name"
                :background="researchResult.getQuestionVariantAnswer(variant.id) ? '#DFF2F8' : '#ffffff'"
                margin-right="10px" height="42px" width="100%" font-size="16px" border-radius="5px" color="#343e5c"
                :with-icon="false" @click="addAnswerToQuestionVariant(variant)">
              </Button>
            </GridContainer>
          </div>
        </template>
